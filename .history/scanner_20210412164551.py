...
async def on_ready():
    ...
    for channelID in channelIDs:
        ...
        if type(channel) == discord.channel.TextChannel:
            ...
            filename = os.path.join(foldername, channel.name+'.txt')

            files.append(filename)
            # print(data)
            with open(filename, 'w') as channeltxt:
                channeltxt.write('\n'.join(messages))
            # data.to_csv('{}/{}.csv'.format(folder, channel.name))
    

    with open(os.path.join(transcriptsfolder, 'index.txt'), 'w') as indexfile:
        indexfile.write('\n'.join(files))
    ...
    try:
        ...
        async for msg in channel.history(limit=limit):
            if msg.author != client.user:
                ...
                messages[0] = messages[0][:99] # limit messages to 100 in length
